...
<p>www.corona-stats.co.za launched a free API for historical South African
...
<h1>2.Data access </h1>
<p>To extract data from the API, R programming language packages:httr and Jsonlite were used. Some chunks of code are written as comments because when the notebook was complied the API key had expired.
</p>
```{r}
#library(httr)
#library(jsonlite)
...
<p>To check whether the request was successful the status code has to be
...
```
<p>Converting the response to usable format
</p>
```{r}
...
<p>The Covid19 data is in the RSA list.RSA contains 22 lists of South
Africa Covid19 data.
</p>
...
#mySA_Covid19data = data.frame(Date=Dates1,Total_Cases=Cases,Total_Recoveries=Recoveries,
                              ...
                              #Daily_Cases,Daily_Deaths,Daily_Vaccinations,Cases_14day_average,Deaths_14day_average,Vaccinations_14day_average)
...
<p>All the provincial dataframes were combined into 1 dataset, Sa_ProvincialCOVID19.
</p>
```{r}
library(readxl)
library(dplyr)
...
Sa_ProvincialCOVID19 = read_excel("~/GitHub/South-Africa-COVID19/Sa_ProvincialCOVID19.xlsx")
mydata = Sa_ProvincialCOVID19

#COVID19 National data
data = read_excel("~/GitHub/South-Africa-COVID19/SaCovid19.xlsx")
```


<h2>A. Basic data profiling</h2>
<p>The number of rowns and number of columns of Provincial COVID19 dataset
</p>
```{r}
nrow(mydata)
ncol(mydata)
...
<p>The number of rows and columns of the COVID19 National dataset:
</p>
```{r}
nrow(data)
ncol(data)
...
<p>The data types of the COVID19 provincial data
</p>
```{r}
library(funModeling)
df_status(mydata)
```
<p>Data types for the COVID19 National dataset
</p>
```{r}
df_status(data)
```

<p>Columns q_zeros and p_zeros show the number of zeros and percentage of
...
data$Daily_Deaths = as.numeric(data$Daily_Deaths)
data$Total_Cases = as.numeric(data$Total_Cases)
data$Total_Recoveries = as.numeric(data$Total_Recoveries)
data$Total_Deaths = as.numeric(data$Total_Deaths)
data$Daily_Cases = as.numeric(data$Daily_Cases)
data$Cases_14day_average = as.numeric(data$Cases_14day_average)
data$Deaths_14day_average = as.numeric(data$Deaths_14day_average)
data$Total_Tests = as.numeric(data$Total_Tests)
data$Total_Vaccinations = as.numeric(data$Total_Vaccinations)
data$Vaccinations_14day_average = as.numeric(data$Vaccinations_14day_average)
data$Daily_Vaccinations = as.numeric(data$Daily_Vaccinations)

df_status(data)

```

<p>All the column data types have been corrected. To find the statistical
...
<p>There appears to be an error in the Daily_Deaths,Daily_Cases and
Deaths_14day_average columns. The minimum values are negative and that
needs to be rectified. All the negative numbers will be replaced by the
...
summary(data)
```
<p>From the above summary the minimum daily vaccinations has a negative value.
</p>
```{r}
data$Daily_Vaccinations = abs(data$Daily_Vaccinations)
summary(data)
```
<p> The variables Daily_cases,Cases_14day_average,Deaths_14day_average and Vaccinations_14day_average have missing values. The missing values will be replaced by the median of the variable
</p>
```{r}
data$Daily_Cases[which(is.na(data$Daily_Cases))] = median(data$Daily_Cases,na.rm = T)
data$Cases_14day_average[which(is.na(data$Cases_14day_average))] = median(data$Cases_14day_average,na.rm = T)
data$Deaths_14day_average[which(is.na(data$Deaths_14day_average))] = median(data$Deaths_14day_average,na.rm = T)
data$Vaccinations_14day_average[which(is.na(data$Vaccinations_14day_average))] = median(data$Vaccinations_14day_average,na.rm = T)

summary(data)
```

<h2>B. Univariate Analysis</h2>
...
hist(mydata_sorted$Daily_Cases, main = "Daily Cases",
     ...
     ylab = "Frequency",
     col = "blue")
plot(density(mydata_sorted$Daily_Cases), main = "Daily Cases distribution", col = "red",lwd = 2)
...
summary(mydata_sorted$Daily_Cases)
```
<p>The daily cases are skewed to the left as shown in the descriptive
...
```
<p>The first peak corresponds to the first COVID19 outbreak in May/June
...
the outbreak of the Delta variant, which has a higher transmissibility.
Torwards the end of 2021 and beginning of 2022 there was an outbreak of
the Omicron variant. This corresponds to the last peak in the plot
above.
</p>
<p>Daily cases for National dataset:
</p>
```{r}
summary(data$Daily_Cases)
par(mfrow=c(1,3))
hist(data$Daily_Cases, main = "Daily Cases",
     xlab = "Daily cases",
     ylab = "Frequency",
     col = "blue")
plot(density(data$Daily_Cases), main = "Total Cases distribution", col = "red",lwd = 2)
boxplot(data$Daily_Cases,col = "blue",main="Boxplot:Daily Cases")
```
<p>It is possible the maximum number of cases recorded in one day is 37875. Therefore the values above the 1.5IQR are possibly extreme values not errors.
</p>
```{r}
plot(data$Date,data$Daily_Cases,type = "l",lwd = 1, col = "#cc0000",main = "National Daily COVID19 cases with time",
     xlab = "Date",
     ylab = "Daily COVID19 cases")
```

<h3>ii Daily Deaths</h3>
<Daily deaths: Provincial dataset>
```{r}
par(mfrow=c(1,3))
mydata_sorted$Daily_Deaths = abs(mydata_sorted$Daily_Deaths)
...
hist(mydata_sorted$Daily_Deaths, main = "Daily Deaths",
     ...
     ylab = "Frequency",
     col = "blue")
plot(density(mydata_sorted$Daily_Deaths), main = "Daily Deaths distribution", col = "red",lwd = 2)
boxplot(mydata_sorted$Daily_Deaths,col = "blue",main="Boxplot:Daily Deaths")

```
<p>The daily deaths are skewed to the left as shown in the descriptive
stats, histogram, density plot and boxplot. There appears to be outliers
as shown by the boxplot but these values are extreme values which
...
```
<p>Daily deaths: National dataset
</p>
```{r}
par(mfrow=c(1,3))
hist(data$Daily_Deaths, main = "Daily Deaths",
     xlab = "Daily deaths",
     ylab = "Frequency",
     col = "blue")
plot(density(data$Daily_Deaths), main = "Daily Deaths distribution", col = "red",lwd = 2)
boxplot(data$Daily_Deaths,col = "blue",main="Boxplot:Daily Deaths")

```
```{r}
plot(data$Date,data$Daily_Deaths,type = "l",lwd = 1, col = "blue",main = "Daily COVID19 deaths with time",
     xlab = "Date",
     ylab = "Daily COVID19 deaths")
```
...
```
<h3>viii. Daily Vaccinations</h3>
```{r}
par(mfrow=c(1,3))
data$Daily_Vaccinations = abs(data$Daily_Vaccinations)
hist(data$Daily_Vaccinations, main = "Deaths:14day average",
     xlab = "Deaths 14day average",
     ylab = "Frequency",
     col = "blue")
plot(density(data$Daily_Vaccinations), main = "Deaths 14day average distribution", col = "red",lwd = 2)
boxplot(data$Daily_Vaccinations,col = "blue",main="Boxplot:Deaths 14day average")
```
```{r}
summary(data$Daily_Vaccinations)
```
```{r}
plot(data$Date,data$Daily_Vaccinations,type = "l",lwd = 1, col = "blue",main = "Daily COVID19 vaccinations with time",
     xlab = "Date",
     ylab = "Daily COVID19 vaccnations")
```


<h2>C.Multivariate Analysis</h2>
```{r}
...
```
<p>Though Gauteng has had the highest number of total COVID19 cases, more
...
```
<p>The boxplots above give a relative comparison of the total cases and
...
plot(mydata_sorted$Total_Cases,mydata_sorted$Total_Deaths,
     ...
     ylab = "Total deaths")
```
<p>There is a direct correlation between the total cases and total
...
plot(gauteng$Total_Cases,gauteng$Total_Deaths,
     ...
     ylab = "Total deaths")
```
<p>We can define death rate as the number of deaths divided by the number
...
<h2>D.Data Visualization using Tableau</h2>
<p>(i)Total confirmed cases and deaths over time dashboard:</p>
<p>https://public.tableau.com/app/profile/mufaro.chinake/viz/SouthAfricaCOVID19ConfirmedCases/Dashboard1?publish=yes
</p>
<p>(ii)Daily cases and deaths over time:</p>
<p>https://public.tableau.com/app/profile/mufaro.chinake/viz/SouthAfricaCOVID19DailyCasesandDeaths/Dashboard1?publish=yes
</p>